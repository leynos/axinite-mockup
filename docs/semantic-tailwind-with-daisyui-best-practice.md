# Quick guide: semantics + utilities with Tailwind v4, daisyUI v5, and Kobalte

This is a pragmatic layering recipe for building accessible, themeable UIs with
...
For state-specific tweaks, prefer utility colours over toggling component
...
Kobalte sets **state** through data attributes. Tailwind v4 can target them
directly.

```tsx
// Dialog with daisyUI look, Kobalte behaviour
...
Other handy Kobalte states:

- `data-[highlighted]` for focused/hovered items in menus/lists.
- `data-[pressed]` for toggle buttons.
    ...
    <article class="card bg-base-100 shadow-sm">
      <div class="card-body gap-3">
        <header class="flex items-center justify-between">
          ...
          <ToggleButton.Root
            class="btn btn-ghost data-[pressed]:bg-primary data-[pressed]:text-primary-content"
            aria-label="Enable plan"
          >
            Enable
          </ToggleButton.Root>
        </header>
        <p class="prose-muted">Unlimited projects, email support.</p>
...
**Goal:** one place to name the look-and-feel, many places to use it — across
Tailwind utilities, daisyUI roles, and Kobalte state styling.
...
```html
<!-- Colour utilities from project tokens -->
...
Kobalte sets `data-state`, `data-disabled`, `data-highlighted`, and
component-specific state attributes. Pair those with tokenized utilities for
