...
In this snippet, the Bun test quickly renders a Solid component into a fake
DOM and makes assertions. The usage of `document.querySelector` and standard
DOM APIs is possible thanks to Happy DOM. Tests like this run blazingly fast
(no real browser, no JSDOM overhead) and can be run hundreds of times a minute
during development.
...
most Solid component output and event simulation, Happy DOM is sufficient and
...
- **Add Dependencies:** Install `axe-core` (or `jest-axe`),
  ...
  devDependencies for the Node tests. Optionally, add `tsgo` or configure
  `tsc` for quick compilation of tests.
